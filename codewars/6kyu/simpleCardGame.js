...
function winner(deckSteve, deckJosh) {
    ...
    for (let i = 0; i < deckSteve.length; i++){
      switch(deckSteve[i]){
          ...
          break;
          default:
          steve.push(Number(deckSteve[i]));
          break;
      };
    };
    for (let i = 0; i < deckJosh.length; i++){
      switch(deckJosh[i]){
          case 'T':
          josh.push(10)
          break;
    ...
    return steveCount > joshCount ? `Steve wins ${steveCount} to ${joshCount}` : joshCount > steveCount ? `Josh wins ${joshCount} to ${steveCount}` : `Tie`
}