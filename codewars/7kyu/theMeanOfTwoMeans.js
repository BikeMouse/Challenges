...
function getMean(arr, x, y) {
    if (x <= 1 || x > arr.length) return -1
    if (y > arr.length) return -1
    let xArray = arr.slice(0, x)
    let yArray = []
    while (yArray.length !== y) {
        yArray.push(arr[arr.length - 1])
        arr.pop()
    }
    return ((xArray.reduce((acc, c) => acc + c, 0) / xArray.length) + (yArray.reduce((acc, c) => acc + c, 0) / yArray.length)) / 2
}