...
Challenge: You are given a list of numbers. The numbers each repeat a certain 
number of times. Remove all numbers that repeat an odd number of times while 
...
function oddOnesOut(nums) {
    ...
    for (let i = 0; i < nums.length; i++) {
        if (nums.filter((e) => e == nums[i]).length % 2 == 0) output.push(nums[i])
    }
    return output
}