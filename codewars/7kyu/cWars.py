...
# and last name; however in some cultures (for example in South India) there 
# may be more than one middle name.

# Write a function that takes the full name of a person, and returns 
# the initials, separated by dots ('.'). The initials must be uppercase. 
# The last name of the person must appear in full, with its first letter in 
# uppercase as well.
...
def initials(name):
    ...
    for i in range(len(arr)):
        if i == len(arr)-1:
            output += arr[i][0].upper()+arr[i][1:]
        else:
            output += arr[i][0].upper()+'.'
    return output