...
function conjugate(verb) {
    let word = verb.slice(0, verb.length - 2);
    let suffix = verb.slice(-2);
    let conjugated = ['I', 'You', 'HeSheIt', 'We', 'You', 'They'];
    let er = ['o', 'es', 'e', 'emos', 'éis', 'en'];
    let ar = ['o', 'as', 'a', 'amos', 'áis', 'an'];
    let ir = ['o', 'es', 'e', 'imos', 'ís', 'en'];
    for (let i = 0; i < conjugated.length; i++) {
        if (suffix == 'er') {
            conjugated[i] = word + er[i];
        } else if (suffix == 'ar') {
            conjugated[i] = word + ar[i];
        } else {
            conjugated[i] = word + ir[i];
        };
    };
    let obj = {};
    obj[`${verb}`] = conjugated;
